...
export class TidelineRoadSignals {
 ...
 constructor(private readonly course:TidelineCourse) {
  ...
  const paint=hardwareMaterial(hardwareAtlas('signage'),0xa9aa88,.025);paint.transparent=true;paint.opacity=.82;paint.depthWrite=false;paint.polygonOffset=true;paint.polygonOffsetFactor=-1;
  ...
  for(const zone of TIDELINE_ABILITY_CONFIG.launchZones) {
   ...
   for(let p=zone.from;p<zone.to;p+=5/course.length) {
    ...
    for(const side of [-1,1]) {
     const geometry=atlasTile(new THREE.PlaneGeometry(3.9,.68),3);geometry.rotateX(-Math.PI/2);geometry.rotateY(side*.58);geometry.translate(side*1.65,.052,0);geometry.applyMatrix4(basis);geometry.translate(...sample.position.toArray());chevrons.add(geometry);
   ...
   const row=new THREE.InstancedMesh(new THREE.BoxGeometry(.36,.065,.9),lamps,matrices.length);row.name='launch_flush_chasing_lamps';matrices.forEach((m,i)=>{row.setMatrixAt(i,m);row.setColorAt(i,new THREE.Color(.4,.4,.4));});row.computeBoundingSphere();root.add(row);
   const stamp=this.stencil('SURGE');const at=course.sample(zone.from+.002);stamp.position.copy(at.position).addScaledVector(at.up,.057);stamp.quaternion.setFromRotationMatrix(new THREE.Matrix4().makeBasis(at.right,at.up,at.tangent.clone().negate()));root.add(stamp);
  ...
  for(const side of [-1,1]) {
   ...
   const glass=new THREE.MeshLambertMaterial({color:0x518788,transparent:true,opacity:.23,emissive:0x172d2b,emissiveIntensity:.12,depthWrite:false,side:THREE.DoubleSide});
   root.add(covers.mesh('sealed_channel_glass',glass));
   const row=new THREE.InstancedMesh(new THREE.BoxGeometry(.85,.045,.32),hardwareMaterial(null,0xb9dfc9,.8),samples.length);row.name='current_travelling_lamps';
 ...
 update(time:number,reduced:boolean,progress:number,seed:number,lap:number,ready:boolean,surge:boolean):void {
  for(const strip of this.strips) {
   const distance=Math.abs((((strip.from+strip.to)/2-progress+1.5)%1)-.5)*this.course.length;strip.root.visible=distance<300;
  ...
  for(const current of this.currents) {
   ...
   for(let i=0;i<current.samples.length;i++) {
    ...
    this.color.setScalar(active?.35+wave:.09);current.lamps.setColorAt(i,this.color);
