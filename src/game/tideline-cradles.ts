...
import {distantDevice,disposeDistantDevice} from './tideline-device-lod';
...
interface Cradle {
  ...
  beacon:THREE.Mesh; distant:Record<'surge'|'shield',THREE.Group>|null; devices:Record<'surge'|'shield',PowerKitVisual>|null;
...
export class TidelineCradles {
  ...
  constructor(private readonly course:Pick<RaceCourse,'sample'|'length'>,private readonly pickups:readonly PickupDefinition[],assetUrl?:string,atlasRoot?:string,stencil?:string) {
    this.root.name='tideline_device_cradles';
    const steel=hardwareMaterial(hardwareAtlas('metal',atlasRoot)),sign=hardwareMaterial(hardwareAtlas('signage',atlasRoot));
    let labelMaterial=sign;
    if(stencil){const canvas=document.createElement('canvas');canvas.width=512;canvas.height=128;const context=canvas.getContext('2d')!;context.fillStyle='#424f36';context.fillRect(0,0,512,128);context.fillStyle='#d9c89f';context.font='bold 78px monospace';context.textAlign='center';context.fillText(stencil,256,92);const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;labelMaterial=hardwareMaterial(texture);}
    const bulb=hardwareMaterial(hardwareAtlas('emissive',atlasRoot),0xffffff,1.1);
    for(const [index,pickup] of pickups.entries()) {
      ...
      // The beacon sits on a metal foot with rivet-scale fasteners, not bare glass.
      rail.add(atlasTile(new THREE.CylinderGeometry(.30,.30,.12,12),1).translate(pickup.lateral,8.72,0));
      for(const x of [-.21,.21])for(const z of [-.21,.21])rail.add(atlasTile(new THREE.CylinderGeometry(.012,.016,.014,8),1).translate(pickup.lateral+x,8.79,z));
      ...
      const label=new THREE.Mesh(stencil?new THREE.PlaneGeometry(3.5,.95):atlasTile(new THREE.PlaneGeometry(3.5,.95),2),labelMaterial);label.name='cradle_DEVICE_stencil';label.position.set(pickup.lateral,6.6,.43);label.userData.maximumLetterHeight=.48;root.add(label);
      ...
      this.root.add(root);this.cradles.push({root,swing,carrier,cables,jaws,beacon,distant:null,devices:null,sparks,collectedAt:-100,wasAvailable:true});
    }
    this.ready=PowerKit.load(true,assetUrl).then(kit=>{
      if(this.disposed){kit.dispose();return;}this.kit=kit;
      for(const cradle of this.cradles){cradle.devices={surge:kit.createPickupVisual('surge'),shield:kit.createPickupVisual('shield')};cradle.distant={surge:distantDevice(kit.templates.surge),shield:distantDevice(kit.templates.shield)};for(const device of Object.values(cradle.devices))cradle.carrier.add(device.root);cradle.carrier.add(...Object.values(cradle.distant));}
    });
  }
  update(time:number,reduced:boolean,states:readonly PickupAppearance[],progress:number):void {
    for(const [index,cradle] of this.cradles.entries()) {
      ...
      cradle.root.visible=distance<185;
      ...
      if(cradle.devices)for(const kind of ['surge','shield'] as const){const device=cradle.devices[kind];device.root.visible=state.available&&kind===state.kind&&distance<65;if(cradle.distant)cradle.distant[kind].visible=state.available&&kind===state.kind&&distance>=65;device.update(time,reduced,state.charge,0);}
  ...
  dispose():void {if(this.disposed)return;this.disposed=true;for(const cradle of this.cradles)if(cradle.distant)Object.values(cradle.distant).forEach(disposeDistantDevice);this.kit?.dispose();}
}
