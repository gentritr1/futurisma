...
export class AscensionPaintedEnvironment implements RaceEnvironment {
 ...
 private constructor(readonly root:THREE.Group){
  ...
  });this.stats.meshes=this.meshes.length;this.stats.materials=materials.size;this.stats.textures=textures.size;root.add(this.sky.root);
 }
 static async load(){const environment=new AscensionPaintedEnvironment((await new GLTFLoader().loadAsync('/assets/ascension/painted.glb')).scene);await environment.sky.ready;return environment;}
