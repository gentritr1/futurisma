import {AscensionEgrets} from './ascension-egrets';
...
export class AscensionPaintedEnvironment implements RaceEnvironment {
 ...
 private readonly egrets:AscensionEgrets|null;
 ...
 private constructor(readonly root:THREE.Group,course?:AscensionCourse){
  ...
  });this.egrets=course?new AscensionEgrets(root,course):null;mergeAscensionStaticPaint(root);this.meshes.length=0;this.stats.triangles=0;
 ...
 updateVisibility(camera:THREE.Camera){
  camera.updateMatrixWorld(true);this.effects?.update(camera);this.egrets?.update(new URLSearchParams(location.search).get('motion')==='reduce');this.root.updateMatrixWorld(true);this.updateInstances(camera);this.mangroves.update(camera,this.effects?.root.userData.eventState);this.terrain.update();
