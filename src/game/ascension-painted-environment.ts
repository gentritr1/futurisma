...
export class AscensionPaintedEnvironment implements RaceEnvironment {
 ...
 private constructor(readonly root:THREE.Group){
  ...
  });this.stats.meshes=this.meshes.length;this.stats.materials=materials.size;this.stats.textures=textures.size;root.add(this.sky.root,this.mangroves.root,this.terrain.root);
 }
 static async load(){const environment=new AscensionPaintedEnvironment((await new GLTFLoader().loadAsync('/assets/ascension/painted.glb')).scene);await Promise.all([environment.sky.ready,environment.mangroves.ready,environment.terrain.ready]);return environment;}
 updateVisibility(camera:THREE.Camera){
  this.mangroves.update(camera);this.terrain.update();
