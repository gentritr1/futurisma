...
export class AscensionSky {
 ...
 constructor(){
  ...
  this.root=new THREE.Mesh(new THREE.SphereGeometry(560,40,20),new THREE.ShaderMaterial({uniforms:{panorama,haze:this.haze,launchGlow:this.glow},side:THREE.BackSide,depthWrite:false,depthTest:false,
   vertexShader:'varying vec3 direction; void main(){direction=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',
   fragmentShader:`uniform float launchGlow;uniform sampler2D panorama;uniform vec3 haze;varying vec3 direction;
    ...
    // Warm reflected flame light under the painted cloud deck, before shared tone mapping.
    gl_FragColor.rgb+=vec3(1.,.33,.07)*launchGlow*.65*smoothstep(.04,.18,d.y)*(1.-smoothstep(.45,.8,d.y));
 ...
 update(camera:THREE.Camera,fog=new THREE.Color(0x8e9a8e),launchGlow=0){this.glow.value=launchGlow;this.root.position.copy(camera.position);fog.getRGB(this.haze.value,THREE.SRGBColorSpace);}
}
