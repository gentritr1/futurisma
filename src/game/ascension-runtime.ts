...
export class AscensionRuntime implements CircuitRuntime {
 ...
 constructor(readonly course:AscensionCourse,private readonly input:InputController){
  ...
  for(const progress of [150/course.length,.80]){
   ...
   for(const x of [-15,15]){const mast=new THREE.Mesh(new THREE.BoxGeometry(1,19,1),steel);mast.position.set(x,9.5,0);root.add(mast);masts.push(mast);}
   const canvas=document.createElement('canvas');canvas.width=1024;canvas.height=256;
   const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;
   const sign=new THREE.Mesh(new THREE.PlaneGeometry(30,7.5),new THREE.MeshLambertMaterial({map:texture,emissiveMap:texture,emissive:0xffffff,emissiveIntensity:.45,side:THREE.DoubleSide}));sign.position.y=16;root.add(sign);signs.push(sign);root.name='ascension_countdown_board';
 ...
 updateHud(progress:number){
  ...
  this.output.textContent=JSON.stringify({script:'src/game/ascension-runtime.ts',seed:clock.seed,tick:clock.tick,progress,schedule:config,state:clock.state,events:clock.events});
 }
