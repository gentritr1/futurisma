...
import route from "./data/nightshift/route.json";
import rivalPace from "./data/nightshift/rival-pace.json";
...
const LIGHTING: CourseLightingProfile = {
  sky: new THREE.Color(0x648aab), ground: new THREE.Color(0x293148),
...
const FOG: FogProfile = { color: new THREE.Color(0x263548), density: .0032 };
...
export class NightshiftCourse implements RaceCourse {
  readonly kind = "nightshift" as const;
  ...
  constructor() {
    this.group.name = "nightshift_street_circuit";
  ...
  private createStreet(): THREE.Mesh {
    ...
    const mesh=new THREE.Mesh(geometry,material); mesh.name="nightshift_rain_polished_asphalt";
    return mesh;
  }
  ...
  private createFurniture(): THREE.Group {
    ...
    const lamps=new THREE.InstancedMesh(new THREE.BoxGeometry(.22,.07,1.5),
      new THREE.MeshBasicMaterial({color:0x87b7b5,toneMapped:false}),count*2);
    ...
    for(let i=0;i<count;i++) {
      ...
      for(let j=0;j<2;j++) {
        ...
        transform.position.y+=.35; transform.updateMatrix(); lamps.setMatrixAt(i*2+j,transform.matrix);
      }
    }
