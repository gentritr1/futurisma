"""§9: unchanged grade and FFT; separate shallows and all-visible-water stats."""
...
def blobs(mask):
    ...
    return largest
def stats(a,mask):
    if not mask.any():return dict(pixels=0,chroma=None,p50=None,p99=None,maxLuma=None,whitePct=None,maxWhiteBlob=0,autocorrelationPeak=0)
    ...
    lab=grade.lab(rgb)
    return dict(maxLuma=float(luma.max()),pixels=len(rgb),chroma=float(np.hypot(lab[:,1],lab[:,2]).mean()),p50=float(np.median(luma)),p99=float(np.percentile(luma,99)),whitePct=float(100*white.sum()/len(rgb)),maxWhiteBlob=blobs(white),autocorrelationPeak=autocorrelation_peak(white))
rows=[]
for pose in ['court','reef']:
 for blend in ['000','100']:
    ...
    for kind in ['sea','road','shallows','foam']:
    ...
    # Union of all visible water surfaces, including out-of-band near water.
    masks=[]
    for kind in ['sea','shallows','foam']:
        isolated=np.array(Image.open(folder/f'blend-{blend}-{kind}.png').convert('RGB'))
        masks.append(np.any(isolated!=blank,axis=2)&np.all(a==isolated,axis=2))
    row['allWater']=stats(a,np.logical_or.reduce(masks))
    rows.append(row)
(base/sys.argv[1]/'review-measurements.json').write_text(json.dumps(rows,indent=2))
for row in rows:print(row['pose'],row['blend'],'chroma',row['chromaMean'],'p99/range',row['p99'],row['range'],'sea',row['sea'],'road',row['road'], 'shallows',row['shallows'], 'allWater',row['allWater'])
