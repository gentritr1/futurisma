...
export async function instrument(page,{grip,basinWidth,stage}={}){
 ...
 page.on('request',async request=>{
  try{if(stage==='baseline'&&new URL(request.url()).pathname==='/assets/dreamisland/painted.glb'){
   await request.respond({status:200,contentType:'model/gltf-binary',body:readFileSync('/tmp/di-b59-painted.glb')});
  }else if(['baseline','ao-only','g2-light'].includes(stage)&&new URL(request.url()).pathname==='/assets/dreamisland/props.glb'){
   await request.respond({status:200,contentType:'model/gltf-binary',body:readFileSync('/tmp/di-b59-props.glb')});
  }else if(['baseline','ao-only'].includes(stage)&&new URL(request.url()).pathname==='/src/game/dreamisland-materials.ts'){
   const response=await fetch(request.url());const code=(await response.text()).replace('mix(.22,1.,diDayLightNight)','mix(1.,1.,diDayLightNight)').replace('mix(1.2,1.,diDayLightNight)','mix(1.,1.,diDayLightNight)');
   await request.respond({status:200,contentType:'text/javascript',body:code});
  }else if(new URL(request.url()).pathname==='/src/game/game.ts'){
   ...
   code=code.replace('this.update(delta, input);','if(window.__hold){this.animationFrame=requestAnimationFrame(this.frame);return;} this.update(delta, input);');
