...
**THE MAP IS SCOPE-COMPLETE (A–D + polish).** Open: user eyeball on polish/eyeball-before-after.png → optional second polish round; device verification (everything is headless desktop Chrome); snapshot/restore still not wired into gameplay (validator-only, by design).
