...
**POLISH-3 (Codex) — E1–E3 DONE, reviewed 2026-09-12, APPROVE; E4 ruled (causeway accepted at the measured 0.213 s; fog/braking target withdrawn — autopilot cannot see fog; controller extension NOT approved).** Sound: six signature clips shipped under a pinned 248,504 B ceiling, surf/night beds synthesised and tone-matched; the clock's hands run from the tick and hit twelve at the strike in every format; power-kit hardware beside each pickup with a deck plate at the trigger. **Budgets now: worst tier 105 draws / 162,334 tris (5 draws spare); shell gzip 277.0 KiB = the ceiling, zero headroom — any further initial-bundle byte forces a deliberate re-baseline.** Evidence art/evidence/dreamisland-v1/polish-3/.

**PHASE F "ALIVE" (2026-09-12/13) — MERGED.** After the first human playtest (empty island, invisible powers, dashboard HUD) the user chose Direction B (screensaver world, hovering capsule pickups, aqua-glass HUD) plus A's timing tower and C's road paint; brief `DREAM-ISLAND-ALIVE.md` with the review rounds appended (§8–§12). Codex: sky-reflected sea with sparse art-directed sparkle, black night sea, wet-road streak, chrome/glass recipes, `props.glb`, `capsule.glb`, glow sprites, colour-only AO. Opus: road paint decal, instanced props (toys at the road edge, 200 bollards), eight shoals, capsule pickups ×2.2 with a 3 m column, the island's own lazy HUD skin (self-hosted Michroma + Share Tech Mono, `<link>` to a self URL so it applies on the dev server), timing tower with live gaps (a real bug fixed in `ui.ts`), five-state power slot with the E keycap and a first-time "HOLD E TO FIRE", world bubble, FOV kick / strike flash / overtake pill. **Budgets now: worst tier 124 draws / 192,268 tris (map ceiling 145 / 220,000); shell gzip 277.2 KiB (ceiling 277.5); island lazy chunk 99.9 KiB gzip pinned at 109.9; fonts 19,028 B pinned at 20,931.** Evidence `art/evidence/dreamisland-v1/alive/{3d,code,combined-final}/` (superseded intermediate frames pruned; every README, table and final frame kept).

**THE MAP IS SCOPE-COMPLETE (A–D, two polish rounds, sound/clock/hardware, Phase F alive).** Next is a person with a controller — on the production preview or the dev server, both now show the skin. Open: user eyeball on polish/eyeball-before-after.png → optional second polish round; device verification (everything is headless desktop Chrome); snapshot/restore still not wired into gameplay (validator-only, by design).
