...
const read = () => page.evaluate(() => { try { const c = JSON.parse(document.getElementById("futurisma-diagnostics")?.textContent || "{}").current || {}; return { d: c.distanceMeters, lat: c.lateralMeters, v: c.speedKph, sector: c.sector, phase: c.phase, apron: c.onApron, grip: c.surfaceGrip, hull: c.hullClearanceMeters, hullMin: c.minimumHullClearanceMeters, hullMax: c.maximumHullClearanceMeters, hover: c.hoverHeightMeters, ndcX: c.hullNdcX, ndc: c.hullNdcY, camLat: c.cameraLateralMeters, lagMax: c.maximumCameraLateralLagMeters, p95: c.p95FrameMs, maxMs: c.maxFrameMs, pullMax: c.maximumCameraOcclusionPullMeters, casts: c.sightCasts, castUs: c.sightCastMicroseconds, ndcMin: c.minimumHullNdcY, ndcMax: c.maximumHullNdcY, camClear: c.cameraSurfaceClearanceMeters, camClearMin: c.minimumCameraSurfaceClearanceMeters, chase: c.chaseMeters, chaseMin: c.minimumChaseMeters, wantChase: c.desiredChaseMeters }; } catch { return null; } });
