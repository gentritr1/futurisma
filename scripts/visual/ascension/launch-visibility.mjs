...
try{
 ...
 await page.evaluateOnNewDocument(({launchTick})=>{
  ...
  window.__ascCaptureFrame=(renderer,args,render)=>{
   ...
   window.saveLaunchFrame({tick:state.tick,secondsAfterT0:(state.tick-launchTick)/120,now:performance.now(),pixels,columnTopElevationDegrees:elevation,cloudGlow:state.cloudGlow,camera:camera.position.toArray(),frame,mask:maskUrl});
  };
 },schedule);
 ...
 const report={script:'scripts/visual/ascension/launch-visibility.mjs',maskDefinition:'Same-frame RGB difference >=3/255, plume enabled versus disabled. Only persistent_launch_smoke changes. One binary mask per captured driving frame.',scope:'Unmodified seeded Works Deluge Road three-lap demo, normal chase camera. Capture passes are not performance benchmarks.',inputHashes:Object.fromEntries(['src/game/ascension-effects.ts','src/game/ascension-mangroves.ts','src/game/ascension-sky.ts','src/game/data/ascension/route.json'].map(f=>[f,createHash('sha256').update(readFileSync(f)).digest('hex')])),windowSeconds:20,expectedRateHz:10,inclusiveEndpointSamples:1,expectedSamples:expected,captured:records.length,residual:records.length-expected,plumeFrames:positive,fraction:positive/records.length,firstTickOffset:first?.tick-schedule.launchTick,lastTickOffset:last?.tick-schedule.launchTick,observedWallWindowSeconds:(last?.now-first?.now)/1000,errors,records};
