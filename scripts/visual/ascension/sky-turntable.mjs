...
const out=process.argv.find(a=>a.startsWith('--out='))?.slice(6)??'art/evidence/ascension-v1/phase-b/sky-turntable';await mkdir(out,{recursive:true});
const tick=process.argv.includes('--postlaunch')?JSON.parse(readFileSync('src/game/data/ascension/schedule.json')).rocketGoneTick+120:0;
...
for(let i=0;i<24;i++){const yaw=i*360/24;await page.goto('http://127.0.0.1:5200/ascension-review.html?progress=.74&tick='+tick+'&yaw='+yaw,{waitUntil:'networkidle0'});await page.waitForSelector('#review-state');const file=String(i).padStart(2,'0')+'.png';await page.screenshot({path:out+'/'+file});records.push({file,yaw,...await page.$eval('#review-state',e=>JSON.parse(e.textContent))});await page.goto('http://127.0.0.1:5200/ascension-review.html?progress=.74&tick='+tick+'&yaw='+yaw+'&skyMask=1',{waitUntil:'networkidle0'});await page.waitForSelector('#review-state');await page.screenshot({path:out+'/mask-'+file});}
