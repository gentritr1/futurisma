...
parser=argparse.ArgumentParser();parser.add_argument('--root',default='art/evidence/ascension-v1/phase-b');root=Path(parser.parse_args().root)
...
report={'script':'scripts/visual/ascension/check-road-floor.py','measurementScript':'scripts/visual/frame-metrics.py','pinScript':'scripts/visual/ascension/pin-road-floor.py','sampling':'8 sectors × 4 static schedule states = 32 frames; no temporal rate. Schedule-frozen road-patch compliance; live speed readability is a separate capture.','accepted':all(r['passes'] for r in rows),'rows':rows}
