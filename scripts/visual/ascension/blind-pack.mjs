...
const root='art/evidence/ascension-v1/phase-b-revision',out=root+'/blind-review';await mkdir(out,{recursive:true});
const key=JSON.parse(readFileSync(root+'/fix-3/acceptance-key.json')).frames;
const order=[7,11,2,9,0,12,4,8,1,10,6,3,5];
let html='<!doctype html><meta charset="utf-8"><title>Ascension image review</title><style>body{background:#18201e;color:#eee;font:18px system-ui;max-width:1280px;margin:32px auto}img{width:100%}textarea{width:95%;height:90px;margin:10px}section{margin:50px 0}</style><h1>Image-only review</h1><p>Use only these images. Record answers before viewing any source or answer key. Responses remain in your browser; copy them into the review report.</p><h2>Landscape</h2><p>Name the tree species or family that the two images communicate. Describe any canopy obstruction of the countdown boards.</p>';
for(const [i,sector] of ['MANGROVE_CUT','CAUSEWAY'].entries()){await copyFile(`${root}/stations/base-${sector}.png`,`${out}/landscape-${i}.png`);html+=`<section><h3>Landscape ${i+1}</h3><img src="landscape-${i}.png"><textarea></textarea></section>`;}
html+='<h2>Focal models</h2><p>Judge each model on visual detail and construction quality. Would you describe it as simpler than a PS2 asset? Explain using visible evidence.</p>';
for(const [i,asset] of ['rocket-platform','crawler-transporter'].entries()){await copyFile(`${root}/fix-2/${asset}.png`,`${out}/model-${i}.png`);html+=`<section><h3>Model ${i+1}</h3><img src="model-${i}.png"><textarea></textarea></section>`;}
