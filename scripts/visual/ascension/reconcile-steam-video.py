...
root=Path(sys.argv[1] if len(sys.argv)>1 else 'art/evidence/ascension-v1/phase-c/steam-speed');capture=json.loads((root/'capture.json').read_text());probe=json.loads(subprocess.check_output(['ffprobe','-v','error','-count_frames','-show_entries','stream=nb_read_frames,r_frame_rate,avg_frame_rate','-show_entries','format=duration','-of','json',str(root/'apron-live.webm')]))
