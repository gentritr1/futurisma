...
const out='art/evidence/ascension-v1/'+(process.argv.includes('--phase-b')?'phase-b':'phase-a')+'/' +(branch?'trench':'deluge')+(reduced?'-reduced':'')+(process.argv.includes('--unbatched')?'-unbatched':'')+(process.argv.includes('--calibrate')?'-calibration':'')+(tier!=='works'||seed!=='3868938316'?'-'+tier+'-'+seed:'');await mkdir(out,{recursive:true});
while(existsSync('/tmp/ascension-review-pause'))await delay(250);
const inputFiles=['src/game/ascension-course.ts','src/game/ascension-runtime.ts','src/game/ascension-schedule.js','src/game/ascension-environment.ts','src/game/ascension-flat-batch.ts','src/game/data/ascension/route.json','src/game/data/ascension/schedule.json','src/game/data/ascension/rival-pace.json',...(process.argv.includes('--phase-b')?['src/game/ascension-painted-environment.ts','src/game/ascension-sky.ts','public/assets/ascension/painted.glb','public/assets/ascension/horizon.png']:['public/assets/ascension/blockout.glb'])];
