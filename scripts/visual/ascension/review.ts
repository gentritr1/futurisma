...
import {AscensionEnvironment} from '../../../src/game/ascension-environment';
...
const environment=await AscensionEnvironment.load(),runtime=new AscensionRuntime(course,new InputController());scene.add(course.group,environment.root);
...
const fog=course.fogAt(progress);scene.fog=new THREE.FogExp2(fog.color,fog.density);scene.background=fog.color.clone();runtime.updateHud(progress);scene.updateMatrixWorld(true);renderer.render(scene,camera);
const state={script:'scripts/visual/ascension/review.ts',scope:'Static blockout evidence station; production course, board and environment geometry, course lights, fog and tone mapping. No running race or vehicle.',progress,tick,trench:parameters.has('trench'),board,boardDistance:board?camera.position.distanceTo(course.sample(board==='1'?150/course.length:.8).position):null,camera:camera.position.toArray(),schedule:course.schedule.state,render:{calls:renderer.info.render.calls,triangles:renderer.info.render.triangles}};
