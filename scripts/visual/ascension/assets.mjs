...
for(const asset of assets){await page.goto('http://127.0.0.1:5200/ascension-assets.html?asset='+(['surge','shield'].includes(asset)?'power-kit&device='+asset:asset)+(process.argv.includes('--crawler-low')?'&low=1':''),{waitUntil:'networkidle0'});await page.waitForSelector('#asset-state');await page.screenshot({path:out+'/'+asset+'.png'});records.push(await page.$eval('#asset-state',e=>JSON.parse(e.textContent)));}
