...
const out='art/evidence/ascension-v1/'+(process.argv.includes('--phase-b')?'phase-b':'phase-a')+(process.argv.includes('--trench-tour')?'/trench-tour':'/stations');await mkdir(out,{recursive:true});
...
try{
 ...
 if(process.argv.includes('--trench-tour')){stations.length=0;for(let i=0;i<10;i++)stations.push({id:'TRENCH_'+String(i).padStart(2,'0'),progress:route.shortcut.from+(route.shortcut.to-route.shortcut.from)*(i+.5)/10,trench:true});}
