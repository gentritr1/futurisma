...
for(const name of names){
 ...
 const uniqueTriangles=triangles,uniquePrimitives=primitives;
 triangles=0;primitives=0;
 for(const node of gltf.nodes){if(node.mesh===undefined)continue;for(const primitive of gltf.meshes[node.mesh].primitives){triangles+=gltf.accessors[primitive.indices??primitive.attributes.POSITION].count/3;primitives++;}}
 ...
 records.push({path,sha256:createHash('sha256').update(bytes).digest('hex'),triangles,primitives,uniqueTriangles,uniquePrimitives,materials});
}
...
writeFileSync(process.argv.find(a=>a.startsWith('--out='))?.slice(6)??'art/evidence/ascension-v1/phase-b/painted-validation.json',JSON.stringify(result,null,2));
